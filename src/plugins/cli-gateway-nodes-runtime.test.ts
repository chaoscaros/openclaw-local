...
describe("resolvePluginCliNodeInvokeGatewayTimeoutMs", () => {
  it("preserves absent, non-finite, and non-positive timeout behavior", () => {
    expect(resolvePluginCliNodeInvokeGatewayTimeoutMs(undefined)).toBeUndefined();
    expect(resolvePluginCliNodeInvokeGatewayTimeoutMs(Number.NaN)).toBeUndefined();
    expect(resolvePluginCliNodeInvokeGatewayTimeoutMs(Number.POSITIVE_INFINITY)).toBeUndefined();
