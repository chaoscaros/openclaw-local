...
function mergeForcedMessageToolAllowlist(
  toolsAllow: string[] | undefined,
  forceMessageTool: boolean,
): string[] | undefined {
  if (!forceMessageTool || !toolsAllow?.length) {
    return toolsAllow;
  }
  const normalized = new Set(toolsAllow.map((entry) => entry.trim().toLowerCase()));
  if (normalized.has("*") || normalized.has("message")) {
    return toolsAllow;
  }
  return [...toolsAllow, "message"];
}
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const toolsAllow = mergeForcedMessageToolAllowlist(params.toolsAllow, forceMessageTool);
    const toolsRaw =
    ...
    const effectivePromptMode = toolsAllow?.length ? ("minimal" as const) : promptMode;
    const effectiveSkillsPrompt = toolsAllow?.length ? undefined : skillsPrompt;
