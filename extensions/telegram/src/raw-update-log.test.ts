...
describe("stringifyTelegramRawUpdateForLog", () => {
  it("redacts private Telegram raw update fields before verbose logging", () => {
    const update = {
      ...
      callback_query: {
        id: "callback-id",
        from: { id: 7777, is_bot: false, first_name: "Bob", username: "bob_private" },
  ...
  it("keeps old log safety guards for long arrays, long strings, and cycles", () => {
    const cyclic: { update_id: number; values: string[]; nested?: unknown } = {
      update_id: 7,
    ...
    const rawLog = stringifyTelegramRawUpdateForLog({
      ...cyclic,
      safe_note: "x".repeat(600),
    });

    expect(rawLog).toContain('"update_id":7');
    expect(rawLog).toContain("...(2 more)");
    expect(rawLog).toContain(`${"x".repeat(500)}...`);
    expect(rawLog).toContain("[Circular]");
  });
});
