...
import { loadDiscoveredWorkspaceSkillEntries } from "./skills.js";
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("includes discovered skills in status even when runtime eligibility fails", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-skills-status-"));
    tempDirs.push(workspaceDir);
    await writeSkill({
      dir: path.join(workspaceDir, "skills", "missing-bin-skill"),
      name: "missing-bin-skill",
      description: "Needs a missing binary",
      frontmatterExtra: 'metadata: {"openclaw":{"requires":{"bins":["definitely-not-installed-bin"]}}}',
    });

    const discovered = withEnv({ PATH: "" }, () =>
      loadDiscoveredWorkspaceSkillEntries(workspaceDir, {
    ...
    const skill = report.skills.find((entry) => entry.name === "missing-bin-skill");

    expect(discovered.map((entry) => entry.skill.name)).toContain("missing-bin-skill");
    expect(skill).toBeDefined();
    expect(skill?.eligible).toBe(false);
    expect(skill?.missing.bins).toContain("definitely-not-installed-bin");
  });
