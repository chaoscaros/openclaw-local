...
describe("node exec events", () => {
  ...
  it("suppresses no-match search exec.finished events with empty output", async () => {
    const ctx = buildExecCtx();
    await handleNodeEvent(ctx, "node-2", {
      event: "exec.finished",
      payloadJSON: JSON.stringify({
        runId: "run-search-no-match",
        command: 'run pwd -> search "missing-pattern" in 2>/dev/null',
        exitCode: 1,
        timedOut: false,
        output: "   ",
      }),
  ...
  it("suppresses failed exec.finished events with empty output", async () => {
    const ctx = buildExecCtx();
    await handleNodeEvent(ctx, "node-2", {
      event: "exec.finished",
      payloadJSON: JSON.stringify({
        runId: "run-build-empty-failure",
        command: "run build:stage (repo)",
        exitCode: 1,
        timedOut: false,
        output: "   ",
      }),
