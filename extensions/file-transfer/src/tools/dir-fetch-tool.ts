...
async function listTarPaths(
  ...
  return new Promise((resolve) => {
    ...
    child.stdin.on("error", (error: NodeJS.ErrnoException) => {
      ...
      if (!aborted) {
        aborted = true;
        resolve({ ok: false, reason: `tar -tzf input error: ${String(error)}` });
      }
    });
...
async function listTarTypeChars(
  ...
  return new Promise((resolve) => {
    ...
    child.stdin.on("error", (error: NodeJS.ErrnoException) => {
      ...
      if (!aborted) {
        aborted = true;
        resolve({ ok: false, reason: `tar -tzvf input error: ${String(error)}` });
      }
    });
    child.stdin.end(tarBuffer);
  });
...
async function unpackTar(tarBuffer: Buffer, destDir: string): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.stdin.on("error", (error: NodeJS.ErrnoException) => {
      ...
      fail(new Error(`tar unpack input error: ${String(error)}`));
    });
    child.stdin.end(tarBuffer);
  });
}
