...
export async function recordShortTermRecalls(params: {
  ...
  await withShortTermLock(workspaceDir, async () => {
    ...
    for (const result of relevant) {
      ...
      const conceptTags = deriveConceptTags({ path: normalizedPath, snippet: rawSnippet });
...
export async function recordGroundedShortTermCandidates(params: {
  ...
  await withShortTermLock(workspaceDir, async () => {
    ...
    for (const item of relevant) {
      ...
      const conceptTags = deriveConceptTags({ path: item.path, snippet: item.identitySnippet });
