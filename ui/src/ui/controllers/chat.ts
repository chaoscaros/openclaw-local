...
function mergeOptimisticMessages(
  ...
  for (const message of existing) {
    ...
    const optimisticText = extractComparableMessageText(message);
    const alreadyPresent = merged.some((candidate) => {
      ...
      return (
        normalizeLowercaseStringOrEmpty(candidateRecord.role) === optimisticRole &&
        extractComparableMessageText(candidate) === optimisticText
      );
    });
    if (!alreadyPresent) {
      merged.push(message);
...
export async function abortChatRun(state: ChatState): Promise<boolean> {
  ...
  try {
    if (!runId) {
      await state.client.request("sessions.abort", { key: state.sessionKey });
      return true;
    }
    const result = await state.client.request<{ aborted?: boolean }>("chat.abort", {
      sessionKey: state.sessionKey,
      runId,
    });
    if (result?.aborted === false) {
      await state.client.request("sessions.abort", { key: state.sessionKey, runId });
    }
    return true;
  } catch (err) {
    state.lastError = formatConnectError(err);
