...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("can require complete discovery entry coverage before using entry modules", () => {
    mocks.resolveDiscoveredProviderPluginIds.mockReturnValue(["codex", "kilocode"]);
    mocks.loadPluginManifestRegistry.mockReturnValue({
      plugins: [
        createManifestPlugin("codex"),
        createManifestPluginWithoutDiscovery({ id: "kilocode" }),
      ],
      diagnostics: [],
    });
    mocks.loadSource.mockReturnValue(createProvider({ id: "codex", mode: "catalog" }));
    mocks.resolvePluginProviders.mockReturnValue([createProvider({ id: "kilocode", mode: "catalog" })]);

    expect(
      resolvePluginDiscoveryProvidersRuntime({ requireCompleteDiscoveryEntryCoverage: true }),
    ).toMatchObject([expect.objectContaining({ id: "kilocode", label: "kilocode" })]);
    expect(mocks.resolvePluginProviders).toHaveBeenCalledWith(
      expect.objectContaining({
        bundledProviderAllowlistCompat: true,
      }),
