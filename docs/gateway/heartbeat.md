...
- Timeout: unset heartbeat turns use `agents.defaults.timeoutSeconds` when set.
  Otherwise, they use the heartbeat cadence capped at 600 seconds. Set
  `agents.defaults.heartbeat.timeoutSeconds` or
  `agents.list[].heartbeat.timeoutSeconds` for longer heartbeat work.
