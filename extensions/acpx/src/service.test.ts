...
describe("createAcpxRuntimeService", () => {
  it("caps configured timeout seconds to timer-safe milliseconds", () => {
    ...
    expect(resolveAcpxTimerTimeoutMs(Number.MAX_SAFE_INTEGER)).toBe(ACPX_TIMER_TIMEOUT_MAX_MS);
    expect(resolveAcpxTimerTimeoutMs(Number.MAX_VALUE)).toBe(ACPX_TIMER_TIMEOUT_MAX_MS);
  });
  ...
  it("caps oversized plugin timeouts before constructing the default acpx runtime", async () => {
    ...
    const [options] = acpxRuntimeConstructorMock.mock.calls[0] ?? [];
    expect(options).toHaveProperty("timeoutMs", ACPX_TIMER_TIMEOUT_MAX_MS);
