...
describe("package scripts", () => {
  ...
  it("keeps the fast local gateway launcher available", () => {
    const packageJson = readPackageJson();

    expect(packageJson.scripts.fast).toBe("node scripts/fast-gateway.mjs");
    expect(fs.existsSync("scripts/fast-gateway.mjs")).toBe(true);
  });
