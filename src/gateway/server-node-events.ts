...
function isNoMatchSearchExecEvent(params: {
  command: string;
  compactOutput: string;
  exitCode: number | undefined;
  timedOut: boolean;
}) {
  if (params.timedOut || params.exitCode !== 1 || params.compactOutput.length > 0) {
    return false;
  }
  const normalizedCommand = params.command.replace(/\s+/g, " ").trim().toLowerCase();
  return (
  ...
  switch (evt.event) {
    ...
    case "exec.denied": {
      ...
      if (evt.event === "exec.started") {
        ...
        if (isNoMatchSearchExecEvent({ command, compactOutput, exitCode, timedOut })) {
          return undefined;
        }
        const shouldNotify = timedOut || compactOutput.length > 0;
        if (!shouldNotify) {
          return undefined;
        }
        if (
