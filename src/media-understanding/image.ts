...
export async function describeImagesWithModel(
  ...
  try {
    ...
    if (!isImageModelNoTextError(error)) {
      throw error;
    }

    let sawReasoningOnly = false;
    const retryMessage = await completeImage((payload) => {
      if (hasImageReasoningOnlyResponse(payload as AssistantMessage)) {
        sawReasoningOnly = true;
      }
      return disableReasoningForImageRetryPayload(payload, model);
    });
    if (!sawReasoningOnly && !hasImageReasoningOnlyResponse(retryMessage)) {
      throw error;
    }
    const text = coerceImageAssistantText({
