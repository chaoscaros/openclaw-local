...
public struct GatewayConnectDeepLink: Codable, Sendable, Equatable {
    ...
    public static func fromSetupInput(_ input: String) -> GatewayConnectDeepLink? {
        ...
        if let link = self.fromSetupCode(trimmed) {
            return link
        }
    ...
    /// Parse a device-pair setup code, raw setup JSON, or copied setup text.
    public static func fromSetupCode(_ code: String) -> GatewayConnectDeepLink? {
        let trimmed = code.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return nil }
        if let link = self.decodeSetupPayload(from: Data(trimmed.utf8)) {
            return link
        }
        if let data = self.decodeBase64Url(trimmed),
           let link = self.decodeSetupPayload(from: data)
        {
            return link
        }
        for candidate in self.setupCodeCandidates(in: trimmed) where candidate != trimmed {
            if let data = self.decodeBase64Url(candidate),
               let link = self.decodeSetupPayload(from: data)
            {
                return link
            }
