...
test("lists and patches session store via sessions.* RPC", async () => {
  ...
  const taskModePatched = await directSessionReq<{
    ok: true;
    entry: { mode?: string; taskId?: string };
  }>("sessions.patch", { key: "agent:main:main", mode: "task", taskId: "task-current" });
  expect(taskModePatched.ok).toBe(true);
  expect(taskModePatched.payload?.entry.mode).toBe("task");
  expect(taskModePatched.payload?.entry.taskId).toBe("task-current");

  const normalModePatched = await directSessionReq<{
    ok: true;
    entry: { mode?: string; taskId?: string };
  }>("sessions.patch", { key: "agent:main:main", mode: "normal" });
  expect(normalModePatched.ok).toBe(true);
  expect(normalModePatched.payload?.entry.mode).toBe("normal");
  expect(normalModePatched.payload?.entry.taskId).toBe("task-current");
  ...
  expect(main2?.mode).toBe("normal");
  expect(main2?.taskId).toBe("task-current");
