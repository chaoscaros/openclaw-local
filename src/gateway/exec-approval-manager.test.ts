...
describe("ExecApprovalManager resolved-entry cleanup", () => {
  ...
  it("unrefs the resolved-entry cleanup timer after resolve", async () => {
    ...
    const record = manager.create({ title: "Approve", description: "desc" }, 30_000, "approval-resolve");
    ...
    expect((cleanupTimer as { unref: ReturnType<typeof vi.fn> }).unref).toHaveBeenCalledTimes(1);
  });

  it("unrefs the resolved-entry cleanup timer after expire", async () => {
    const manager = new ExecApprovalManager<{ title: string; description: string }>();
    const record = manager.create({ title: "Approve", description: "desc" }, 30_000, "approval-expire");
